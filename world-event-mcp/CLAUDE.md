...
World Event MCP Server — 89 tools across 30+ domains providing real-time global intelligence from free public APIs. Serves three interfaces: MCP stdio (for Claude Code/Cursor), a live Starlette dashboard with SSE, and a Click CLI with Rich output. Python 3.11+, built with hatchling.
...
pytest
pytest --cov=world_event_mcp
pytest src/world_event_mcp/tests/test_sources.py::test_fetch_market_quotes -v  # single test
...
dashboard/app.py (SSE)  ─┘                                                  │
                                                                   ~/.cache/world-event-mcp/cache.db
```
