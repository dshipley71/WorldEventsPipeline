"""Aviation data sources for world-event-mcp.
...
from OpenSky Network (anonymous access, no API key required).
"""
...
logger = logging.getLogger("world-event-mcp.sources.aviation")
...
# ---------------------------------------------------------------------------
# Domestic / commercial air traffic (OpenSky Network — anonymous)
# ---------------------------------------------------------------------------
...
async def fetch_domestic_flights(fetcher: Fetcher) -> dict:
    """Fetch global air traffic counts from OpenSky Network (anonymous access).

    Queries all airborne aircraft once, then buckets by region and type.
    No API key required; anonymous rate limits apply (~400 API credits/day).

    Args:
        fetcher: Shared HTTP fetcher with caching and circuit breaking.

    Returns:
        Dict with total aircraft, regional breakdown, and busiest origins.
    """
    data = await fetcher.get_json(
        _OPENSKY_STATES_URL,
