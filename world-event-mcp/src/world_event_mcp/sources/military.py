"""Military aviation tracking source for world-event-mcp.

Provides real-time military aircraft tracking via adsb.lol (free community
API, no key required) with OpenSky Network as fallback (anonymous access),
and aircraft detail lookups via hexdb.io (free, no API key).
"""
...
logger = logging.getLogger("world-event-mcp.sources.military")
...
async def _fetch_opensky_military(
    ...
    """Fetch military aircraft from OpenSky (anonymous access, requires filtering)."""
    params: dict[str, str] = {}
    if bbox is not None:
        parts = bbox.split(",")
        if len(parts) == 4:
...
async def fetch_military_flights(
    ...
    falls back to OpenSky Network (anonymous) if unavailable.
    ...
    logger.debug("adsb.lol unavailable, trying OpenSky (anonymous)")
...
async def fetch_theater_posture(fetcher: Fetcher) -> dict:
    ...
    tasks = [_fetch_theater(name, info) for name, info in THEATERS.items()]
