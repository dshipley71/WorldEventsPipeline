"""RSS news aggregation, keyword trending, and GDELT search for world-event-mcp.
...
logger = logging.getLogger("world-event-mcp.sources.news")
...
async def fetch_gdelt_search(
    ...
    No API key required. Supports article list and all timeline modes.

    Args:
        fetcher: Shared HTTP fetcher with caching and circuit breaking.
        query: Search query string.
        mode: ``artlist`` (articles), ``timelinevol`` (normalized volume),
              ``timelinevolraw`` (raw article count), or ``timelinetone``
              (average tone).  All four modes are supported.
        limit: Maximum number of records (artlist mode only; ignored for
               timeline modes which return full date-range series).
        timespan: GDELT timespan string (e.g. ``"90d"``, ``"30d"``).
                  Only meaningful for timeline modes.  Defaults to GDELT's
                  built-in window when None.
    ...
    is_timeline = mode in _timeline_modes

    # Build GDELT API params.
    # maxrecords applies only to artlist; timeline modes use timespan.
    params: dict = {
        "query": query,
        "mode": mode,
        "format": "json",
    ...
    if timespan and is_timeline:
        params["timespan"] = timespan

    safe_query = re.sub(r"[^a-zA-Z0-9_-]", "_", query)[:64]
    # FIX: include timespan and limit in cache key so different call
    # parameters never share a stale cached result.
    safe_timespan = re.sub(r"[^a-zA-Z0-9]", "_", timespan or "default")
    cache_key = f"news:gdelt:{safe_query}:{mode}:{safe_timespan}:{limit}"
    ...
    # timeline modes (timelinevol / timelinevolraw / timelinetone) — return data as-is.
    # All three share identical GDELT JSON structure: {"timeline":[{"series":"...","data":[...]}]}
