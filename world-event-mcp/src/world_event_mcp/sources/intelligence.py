...
free public APIs (World Bank, USGS, GDELT, IODA, OpenSky, NGA) into
country briefs, instability indices, geographic signal convergence,
focal point detection, signal summaries, temporal anomalies, hotspot
escalation, military surge, vessel tracking, and cascade analysis.
...
logger = logging.getLogger("world-event-mcp.sources.intelligence")
...
# ---------------------------------------------------------------------------
# Function 1: Country Intelligence Brief (World Bank data only)
# ---------------------------------------------------------------------------
...
async def fetch_country_brief(
    ...
    """Generate a country intelligence brief using World Bank public data.

    Gathers GDP and inflation data from the World Bank API in parallel.
    No API key required.
    ...
    Returns:
        Dict with World Bank economic data, source, and timestamp.
    """
    ...
    return {
        ...
        "brief": "Data-only summary (LLM enrichment removed; no API key required).",
        "data": {
            "gdp": gdp_values,
...
# ---------------------------------------------------------------------------
# Function 2: Country Instability Index (signal-based, no ACLED)
# ---------------------------------------------------------------------------
...
async def fetch_instability_index(
    ...
    """Compute a Country Instability Index (CII) from free public signals.

    Combines military activity, internet outage indicators, and news
    velocity into a 0-100 composite score.  No API key required.

    When *country_code* is ``None``, returns a simplified index for 10
    focus countries using config-based baselines.
...
async def _instability_single(
    ...
    """Compute CII v2 instability index for a single country using free sources."""
    ...
    # No conflict/protest data without ACLED — score from zeros
    unrest_val = score_unrest(0, 0)
    conflict_val = score_conflict_v2(0, 0)
    ...
    return {
        ...
        **cii,
        "raw_data": {
            ...
            "note": "Conflict/protest signals unavailable (no ACLED key required sources removed)",
        },
...
async def _instability_multi(fetcher: Fetcher, now: datetime) -> dict:
    """Compute CII v2 instability index for focus countries using config baselines."""
    results: list[dict] = []
    for code in _FOCUS_COUNTRIES:
        name = _ISO3_TO_NAME.get(code, code)
        multiplier = get_event_multiplier(code)

        # Without ACLED, unrest/conflict scores are zero
        unrest_val = score_unrest(0, 0)
        conflict_val = score_conflict_v2(0, 0)
        security_val = 0.0
        info_val = 0.0
    ...
    return {
        "countries": results,
        "count": len(results),
        "note": "Multi-country CII v2 using config baselines only. "
                "Conflict/protest signals unavailable (no API key required sources removed). "
                "Use country_code for full security+information domain analysis.",
        "source": "instability-index-v2",
        "timestamp": now.strftime("%Y-%m-%dT%H:%M:%SZ"),
    }
...
# ---------------------------------------------------------------------------
# Function 3: Signal Convergence (USGS only — always free)
# ---------------------------------------------------------------------------
...
async def fetch_signal_convergence(
    ...
    Checks for seismic activity within a radius of known or specified
    hotspot coordinates.  No API key required (USGS public API).

    Args:
        fetcher: Shared HTTP fetcher with caching and circuit breaking.
        lat: Latitude of center point.  If ``None``, scans 5 global hotspot regions.
    ...
    async def _assess_hotspot(name: str, center: tuple[float, float]) -> dict:
        ...
        return {
            ...
            "lat": center_lat,
            "lon": center_lon,
            "signals": {"earthquakes": earthquake_count},
...
# ---------------------------------------------------------------------------
# Function 4: Focal Point Detection (news + military + outages)
# ---------------------------------------------------------------------------

async def fetch_focal_points(fetcher: Fetcher) -> dict:
    """Gather multi-source events and detect focal points.

    Fetches news headlines, military flights, and internet outages in
    parallel, normalizes them into events, and runs focal point detection
    to find entities where multiple signals converge.  No API key required.
...
# ---------------------------------------------------------------------------
# Function 5: Signal Summary (displacement + earthquakes + outages + military)
# ---------------------------------------------------------------------------
...
async def fetch_signal_summary(
    ...
    """Aggregate intelligence signals by country with convergence scoring.

    Fetches USGS earthquakes, IODA outages, military flights, and UNHCR
    displacement in parallel.  No API key required.
...
# ---------------------------------------------------------------------------
# Function 6: Temporal Anomaly Detection (military flights only)
# ---------------------------------------------------------------------------

async def fetch_temporal_anomalies(fetcher: Fetcher) -> dict:
    """Record observations and check for temporal anomalies in military flights.

    Fetches current military flight counts by theater, records each as a
    temporal observation, and reports any that deviate significantly from
    baselines.  No API key required.
...
# ---------------------------------------------------------------------------
# Function 7: Hotspot Escalation Scoring (military only)
# ---------------------------------------------------------------------------

async def fetch_hotspot_escalation(fetcher: Fetcher) -> dict:
    """Score all 22 intel hotspots using military signals.

    For each hotspot, counts military aircraft nearby (+/- 2 deg).
    No API key required.
            ...
            for ac in theater_data.get("countries", []):
                ...
                if len(parts) == 4:
                    try:
                        ...
                        count = theater_data.get("count", 0)
                        countries = theater_data.get("countries", [ac])
                        per_country = count // max(1, len(countries))
    ...
    military_data = await _fetch_global_military()
...
# ---------------------------------------------------------------------------
# Function 8: Military Surge Detection
# ---------------------------------------------------------------------------

async def fetch_military_surge(fetcher: Fetcher) -> dict:
    """Detect military surge anomalies across sensitive regions.

    Uses theater posture data (free, via adsb.lol / OpenSky anonymous)
    and temporal baseline tracking.  No API key required.
...
# ---------------------------------------------------------------------------
# Function 9: Vessel Snapshot at Strategic Waterways
# ---------------------------------------------------------------------------
...
async def fetch_vessel_snapshot(fetcher: Fetcher) -> dict:
    ...
    Uses NGA MSI filtered for naval/vessel keywords near STRATEGIC_WATERWAYS.
    No API key required.
...
# ---------------------------------------------------------------------------
# Function 10: Infrastructure Cascade Analysis
# ---------------------------------------------------------------------------
...
async def fetch_cascade_analysis(
    ...
    Uses NGA MSI cable health data.  No API key required.
