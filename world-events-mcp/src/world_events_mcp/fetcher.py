...
All external HTTP calls in world-events-mcp go through this module.
...
logger = logging.getLogger("world-events-mcp.fetcher")
...
_SOURCE_RATE_LIMITS: dict[str, float] = {
    ...
    "gdelt": float(os.environ.get("WORLD_INTEL_GDELT_MIN_INTERVAL", "6.0")),
}
    ...
    async def _get_client(self) -> httpx.AsyncClient:
        if self._client is None or self._client.is_closed:
            self._client = httpx.AsyncClient(
                ...
                headers={"User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36"},  # FIX: browser UA; bot string caused 403s on Bloomberg/FT/WSJ/Reuters
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if attempt < self.max_retries:
                    # Default linear backoff (1s, 2s, 3s...)
                    wait = 1.0 * (attempt + 1)

                    # 429 handling: respect Retry-After when present, otherwise use the per-source minimum interval.
                    # This is critical for GDELT (and shared IPs) to avoid rapid retries that guarantee repeated 429s.
                    if isinstance(exc, httpx.HTTPStatusError):
                        ...
                        if status == 429:
                            ...
                            min_interval = _SOURCE_RATE_LIMITS.get(source, 5.0)
                            base = retry_after if retry_after is not None else min_interval
                            # Ensure we never retry faster than the source's min interval.
                            wait = max(base, min_interval) * (attempt + 1)
