...
Phase 7: Health, sanctions, elections, social, nuclear, alerts, trends (+10 = 55 tools).
...
from .sources import markets, economic, seismology, conflict, military, infrastructure, maritime, climate, news, intelligence, prediction, displacement, aviation, cyber, space_weather, ai_watch, health, sanctions, elections, social, nuclear, service_status, geospatial, hacker_news, github_trending, arxiv_papers, usa_spending, environmental, usni_fleet, central_banks
...
# Fetcher tuning via env (useful on shared IPs like Colab)
    ...
    Tool(
        ...
        inputSchema={
            ...
            "properties": {
                ...
                "startdatetime": {
                    "type": "string",
                    "description": "Precise search start in YYYYMMDDHHMMSS format (UTC). Must be within last 3 months. Use with enddatetime for exact windows.",
                },
                "enddatetime": {
                    "type": "string",
                ...
                "sourcelang": {
                    "type": "string",
                    "description": "Filter to articles originally in this language. ISO 639 code (e.g. 'zh', 'ru', 'ar', 'es', 'fr'). Appended to query as sourcelang:X.",
                },
                "sourcecountry": {
                    "type": "string",
                    "description": "Filter to press outlets in this country. FIPS-2 code (e.g. 'RS', 'CN', 'IR', 'US'). Appended to query as sourcecountry:X.",
                },
                "theme": {
                    "type": "string",
                    "description": "GDELT GKG theme for broad topic matching. Examples: TERROR, MILITARY, WMD, ELECTION_FRAUD, PROTEST, SANCTION, CYBER_ATTACK. Appended to query as theme:X.",
                },
                "timelinesmooth": {
                    "type": "integer",
                    "description": "Moving-window smoothing steps for timeline modes (1–30). Reduces noise in high-resolution data. Peaks shift slightly right at higher values.",
                },
            },
        },
    ),
...
async def _dispatch(name: str, arguments: dict[str, Any]) -> Any:
    ...
    match name:
        ...
        case "intel_aircraft_details":
            ...
            if not icao24:
                return {"error": "Missing required argument: icao24"}
            return await military.fetch_aircraft_details(fetcher, icao24=icao24)
        ...
        case "intel_aircraft_batch":
            icao24_list = arguments.get("icao24_list")
            if not icao24_list:
                return {"error": "Missing required argument: icao24_list"}
            return await military.fetch_aircraft_details_batch(
                fetcher, icao24_list=icao24_list,
            )
        ...
        case "intel_classify_event":
            ...
            return await fetch_classify_event(fetcher, text=text)
...
async def _run() -> None:
    ...
    try:
        ...
        await fetcher.close()
        cache.close()
        # Close spike detector SQLite connection if it was initialised
        from .analysis.spikes import _detector as _spike_detector
        if _spike_detector is not None:
            _spike_detector.close()
