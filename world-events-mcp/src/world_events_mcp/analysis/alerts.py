from ..utils import utc_now_iso, safe_fetch
"""Alert digest and weekly trends analysis for world-events-mcp.
...
logger = logging.getLogger("world-events-mcp.analysis.alerts")
...
async def fetch_alert_digest(fetcher) -> dict:
    ...
    (
        ...
        safe_fetch(space_weather.fetch_space_weather(fetcher), "space_weather", context="Alert digest"),
        safe_fetch(intelligence.fetch_instability_index(fetcher), "instability", context="Alert digest"),
        safe_fetch(intelligence.fetch_military_surge(fetcher), "military_surge", context="Alert digest"),
        safe_fetch(infrastructure.fetch_cable_health(fetcher), "cable_health", context="Alert digest"),
        safe_fetch(intelligence.fetch_hotspot_escalation(fetcher), "hotspot_escalation", context="Alert digest"),
        safe_fetch(infrastructure.fetch_internet_outages(fetcher), "internet_outages", context="Alert digest"),
    )
