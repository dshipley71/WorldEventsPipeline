...
logger = logging.getLogger("world-events-mcp.analysis.spikes")

_DB_PATH = os.path.join(
    os.path.expanduser("~"), ".cache", "world-events-mcp", "keyword_spikes.db"
)
...
class KeywordSpikeDetector:
    ...
    def detect_spikes(
        ...
        for kw, current_count in keyword_counts.items():
            ...
            variance = m2 / (n - 1)  # Bessel-corrected sample variance (not population variance)
            stddev = variance ** 0.5
...
async def fetch_keyword_spikes(
    ...
    # Build count dict.
    # NOTE: fetch_trending_keywords returns {"word": w, "count": c} items;
    # the key is "word", not "keyword". Using the wrong key caused a silent
    # KeyError that left kw_counts always empty, disabling spike detection.
    kw_counts: dict[str, int] = {}
    for item in keywords:
        word = item.get("word") or item.get("keyword") or ""
        if word:
            kw_counts[word] = item["count"]
