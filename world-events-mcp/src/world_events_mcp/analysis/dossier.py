...
logger = logging.getLogger("world-events-mcp.analysis.dossier")
...
async def fetch_country_dossier(
    ...
    (
        ...
        safe_fetch(fetch_country_brief(fetcher, country_code=iso2), "country_brief", context="Dossier"),
        safe_fetch(fetch_election_calendar(fetcher, country=iso3), "elections", context="Dossier"),
        safe_fetch(fetch_sanctions_search(fetcher, query=iso3, limit=10), "sanctions", context="Dossier"),
        safe_fetch(fetch_news_feed(fetcher, category="all", limit=200), "news_feed", context="Dossier"),
    )
    ...
    # --- Section 3: Markets ---
    markets = {"note": "Market data unavailable (requires paid API)"}
