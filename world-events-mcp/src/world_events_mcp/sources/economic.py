"""Economic indicator data sources for world-events-mcp.

Fetches development indicators from the World Bank public API.
No API key required.
"""
...
logger = logging.getLogger("world-events-mcp.sources.economic")
...
async def fetch_world_bank_indicators(
    ...
    No API key required.  Defaults to GDP, inflation, and unemployment for
    the USA.  Indicators are fetched in parallel.

    Args:
        fetcher: Shared HTTP fetcher with caching and circuit breaking.
        country: ISO 3166-1 alpha-3 country code (e.g. ``USA``, ``GBR``).
        indicators: List of World Bank indicator IDs.  Defaults to
                    GDP (NY.GDP.MKTP.CD), Inflation (FP.CPI.TOTL.ZG),
                    and Unemployment (SL.UEM.TOTL.ZS).

    Returns:
        Dict with ``country``, ``indicators`` (list of dicts with
