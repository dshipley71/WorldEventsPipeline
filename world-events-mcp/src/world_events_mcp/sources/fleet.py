...
logger = logging.getLogger("world-events-mcp.sources.fleet")
...
async def fetch_fleet_report(fetcher) -> dict:
    ...
    (
        ...
        safe_fetch(military.fetch_theater_posture(fetcher), "theater_posture", context="Fleet report"),
        safe_fetch(intelligence.fetch_vessel_snapshot(fetcher), "vessel_snapshot", context="Fleet report"),
        safe_fetch(intelligence.fetch_military_surge(fetcher), "military_surge", context="Fleet report"),
    )
