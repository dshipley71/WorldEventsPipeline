...
logger = logging.getLogger("world-events-mcp.sources.environmental")
...
async def fetch_environmental_events(
    ...
    if data is None or not isinstance(data, dict):
        return {"events": [], "count": 0, "source": "eonet", "timestamp": utc_now_iso()}
...
async def fetch_disaster_alerts(
    ...
    if data is None or not isinstance(data, dict):
        return {"alerts": [], "count": 0, "source": "gdacs", "timestamp": utc_now_iso()}
